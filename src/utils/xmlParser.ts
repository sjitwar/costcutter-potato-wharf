...
export interface XMLProduct {
  ...
  'cost-price': string;
  'rsp': string;
  'category': string; // Add the actual category field from XML
}

export const parseXMLProducts = (xmlContent: string): Product[] => {
  ...
  try {
    ...
    return productsArray.map((xmlProduct: XMLProduct, index: number) => {
      ...
      // Keep other categories as-is (Frozen Fish, Frozen Pizza, etc.)
      ...
      return {
        ...
        image_url: imageUrls[category as keyof typeof imageUrls] || imageUrls.Other,
